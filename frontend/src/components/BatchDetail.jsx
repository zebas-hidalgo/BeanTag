...
export default function BatchDetail({ batchId, onBack, onSubtractDose, onSaveRecipe, onDeleteBatch, onEditBatch, showToast }) {
  ...
  const handleApplyAiRecipe = () => {
    ...
    if (aiRecommendation.notes) {
      setNotes(prev => {
        const cleanPrev = prev.replace(/\[IA:.*?\]/g, '').trim();
        return `[Receta IA: ${aiRecommendation.notes}] ${cleanPrev}`.trim();
      });
    }
    ...
    <div style={{ padding: '14px 14px 90px 14px' }}>
      ...
      <form onSubmit={handleRecipeSubmit}>
        ...
        <div style={{ display: 'flex', gap: '12px', margin: '16px 0', overflowX: 'auto', paddingBottom: '4px' }}>
          {[
            { id: 'V60 (Filtrado)', icon: `${import.meta.env.BASE_URL}icons/v60.jpg`, label: 'V60' },
            { id: 'Espresso', icon: `${import.meta.env.BASE_URL}icons/espresso.jpg`, label: 'Espresso' },
            { id: 'AeroPress', icon: `${import.meta.env.BASE_URL}icons/aeropress.jpg`, label: 'AeroPress' },
            { id: 'Prensa Francesa', icon: `${import.meta.env.BASE_URL}icons/frenchpress.jpg`, label: 'Prensa' }
          ].map(m => (
            <div 
              key={m.id}
              onClick={() => {
